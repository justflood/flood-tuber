...
function(_setup_obs_studio)
  ...
  elseif(OS_MACOS)
    ...
    if("${_deployment_target}" STREQUAL "")
  ...
  execute_process(
    COMMAND
      "${CMAKE_COMMAND}" -S "${dependencies_dir}/${_obs_destination}" -B "${dependencies_dir}/${_obs_destination}/build_${arch}"
      -G "${_cmake_generator}" "${_cmake_arch}" ${_cmake_extra} -DOBS_CMAKE_VERSION:STRING=3.0.0 -DENABLE_PLUGINS:BOOL=OFF
...
function(_check_dependencies)
  ...
  foreach(dependency IN LISTS dependencies_list)
    ...
    # Removed hash comparison for directory deletion
    ...
    elseif(dependency STREQUAL obs-studio)
      ...
      list(APPEND CMAKE_PREFIX_PATH "${dependencies_dir}" "${dependencies_dir}/cmake")
    endif()
